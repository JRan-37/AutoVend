    ...
    <div className="map-wrap" style={{ padding: compact ? 16 : 32 }}>
      {!compact && (
        <div className="meta-row" style={{ marginBottom: 16 }}>
          <span><span className="dot" /> {active.length} active hubs</span>
          <span><span className="dot idle" /> {expansion.length} expansion zones</span>
          <span style={{ marginLeft: "auto" }}>NO GRANULAR ADDRESSES · REGIONAL ONLY</span>
        </div>
      )}
      <svg viewBox="0 0 1000 600" style={{ width: "100%", height: "auto", display: "block" }} aria-label="AutoVend US reach map">
        <defs>
          ...
          <pattern id="hatch" patternUnits="userSpaceOnUse" width="6" height="6" patternTransform="rotate(45)">
            <line x1="0" y1="0" x2="0" y2="6" stroke="var(--ink-4)" strokeWidth="0.6" opacity="0.4" />
          </pattern>
        </defs>
        ...
        {/* Density blobs (active regions) */}
        {active.map(r => (
          <circle key={"d" + r.id} cx={r.x} cy={r.y} r={28 + r.density * 50} fill="url(#dens)" pointerEvents="none" />
        ))}

        {/* Expansion zones (hatched outlines) */}
        {expansion.map(r => (
          <g key={"e" + r.id} style={{ cursor: "pointer" }}
             onMouseEnter={() => setHover(r)} onMouseLeave={() => setHover(null)}
             onClick={() => onSelect && onSelect(r)}>
            <circle cx={r.x} cy={r.y} r="22" fill="url(#hatch)" stroke="var(--ink-4)" strokeWidth="1" strokeDasharray="3 3" />
          </g>
        ))}
        ...
        {active.map(r => {
          ...
          const isHover = hover && hover.id === r.id;
          return (
            <g key={"h" + r.id} style={{ cursor: "pointer" }}
               onMouseEnter={() => setHover(r)} onMouseLeave={() => setHover(null)}
               onClick={() => onSelect && onSelect(r)}>
              <circle cx={r.x} cy={r.y} r={isSel || isHover ? 9 : 6} fill="var(--accent)" stroke="var(--bg)" strokeWidth="2" />
              {(isSel || isHover) && (
                <g>
                  <rect x={r.x + 12} y={r.y - 22} width={r.name.length * 7 + 24} height="20"
                        rx="4" fill="var(--ink)" />
                  <text x={r.x + 18} y={r.y - 8} fill="var(--bg)"
        ...
        {/* Expansion labels (on hover only) */}
        {hover && hover.status === "expansion" && (
          <g>
            <rect x={hover.x + 12} y={hover.y - 22} width={hover.name.length * 7 + 80} height="20"
                  rx="4" fill="var(--ink)" />
            <text x={hover.x + 18} y={hover.y - 8} fill="var(--bg)"
                  style={{ font: "11px var(--font-mono)", letterSpacing: ".03em" }}>
        ...
        {/* Compass / scale */}
        <g transform="translate(40, 540)" style={{ font: "10px var(--font-mono)", fill: "var(--ink-3)", letterSpacing: ".05em" }}>
          <text>N ↑</text>
        </g>
      </svg>
      {!compact && (
